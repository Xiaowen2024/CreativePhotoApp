...
//        }
    ...
    @IBAction private func toggleFiltering() {
        ...
        let alertController = UIAlertController(title: "Choose Filter", message: "Select a filter to apply to the video.", preferredStyle: .actionSheet)
            
            // Add actions for each filter
            for (index, filterRenderer) in filterRenderers.enumerated() {
                let filterAction = UIAlertAction(title: filterRenderer.description, style: .default) { _ in
    ...
    private func applyFilter(at index: Int) {
        videoFilterOn = true
        let stateImage = UIImage(named: "ColorFilterOn")
        self.videoFilterButton.setImage(stateImage, for: .normal)
